...
typedef enum
{
    ZoomSDKNotificationServiceError_Success = 0,//Success.
    ZoomSDKNotificationServiceError_Unknow, //Unknown error.
    ZoomSDKNotificationServiceError_Internal_Error, //Internal error, need retry.
    ZoomSDKNotificationServiceError_Invalid_Token, //Invalid token.
    ZoomSDKNotificationServiceError_Multi_Connect, //Use same user/resource login again on the same device, the previous login application will receive this error.
    ZoomSDKNotificationServiceError_Network_Issue, //Network issue.
    ZoomSDKNotificationServiceError_Max_Duration //Server disconnects the connection if client stayed connected with server for more than 24 hours. Client need to reconnect/login again.
...
typedef enum
{
    ...
    //allow participants to send privilege requests.
...
typedef enum
{
    ZoomSDKReminderType_LoginRequired = 0,  //reminder type of login.
    ...
    ZoomSDKReminderType_SmartSummaryEnableRequestRemind,// reminder type of smart summary enable request.
    ZoomSDKReminderType_QueryDisclaimer,// reminder type of query disclaimer.
    ZoomSDKReminderType_QueryEnableRequestRemind,// reminder type of query enable request.
}ZoomSDKReminderType;

typedef enum
{
    ZoomSDKPreAssignBODataStatus_None, //initial status, no request was sent.
    ZoomSDKPreAssignBODataStatus_Downloading, //download in progress.
    ZoomSDKPreAssignBODataStatus_Download_Ok, //download success.
    ZoomSDKPreAssignBODataStatus_Download_Fail //download fail.
}ZoomSDKPreAssignBODataStatus;

typedef enum
{
    ZoomSDKAutoFramingMode_None,               //no auto framing.
    ZoomSDKAutoFramingMode_Center_Coordinates, //use the video frame’s center point as the center to zoom in.
    ZoomSDKAutoFramingMode_Face_Recognition    //use the detected face in the video frame as the center to zoom in.

}ZoomSDKAutoFramingMode;

typedef enum
{
    ZoomSDKFaceRecognitionFailStrategy_None,                      //no use of the fail strategy.
    ZoomSDKFaceRecognitionFailStrategy_Remain,                   //after face recognition fails, do nothing until face recognition succeeds again.
    ZoomSDKFaceRecognitionFailStrategy_Using_Center_Coordinates, //after face recognition fails, use the video frame’s center point as the center for zoom in.
    ZoomSDKFaceRecognitionFailStrategy_Using_Original_Video      //after face recognition fails, use original video.
